 ...
 * a role yet. Allows them to register as a PROVIDER or request CLIENT access.
 *
 * ADMIN users are created directly via the Clerk dashboard + DB seed.
 */

import { useState } from "react";
import { useRouter } from "next/navigation";
import { useUser } from "@clerk/nextjs";
import { Building2, Truck, ArrowRight, Shield } from "lucide-react";
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (selectedRole === "CLIENT") {
        if (!orgSlug.trim()) {
          setError("Please enter your organization ID provided by your Anzu account manager.");
          setIsLoading(false);
          return;
        }
        body.organizationSlug = orgSlug.trim();
      }
      ...
      // Force Clerk to refresh the session so publicMetadata.role is available
      await user?.reload();
          ...
          {selectedRole === "CLIENT" && (
            <div className="mb-6">
              ...
              <input
                ...
                placeholder="e.g. constructora-bogota"
              ...
              <p className="text-xs text-gray-400 mt-1">
                Your organization ID was provided by your Anzu account manager.
              </p>
            </div>
          )}
